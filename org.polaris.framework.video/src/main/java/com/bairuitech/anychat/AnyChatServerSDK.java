...
{
	public static final int BRAS_MESSAGE_CORESERVERCONN = 10; // /<
																// 与核心服务器的连接消息，wParam为errorcode
	public static final int BRAS_MESSAGE_RECORDSERVERCONN = 11; // /<
																// 与录像服务器的连接消息，wParam为errorcode，lParam为recordserverid
	public static final int BRAS_MESSAGE_LOGINSERVERCONN = 12; // /<
																// 与登录服务器的连接消息，wParam为errorcode，lParam为loginserverid
	public static final int BRAS_MESSAGE_ROOMSERVERCONN = 13; // /<
																// 与房间服务器的连接消息，wParam为errorcode，lParam为roomserverid
	public static final int BRAS_MESSAGE_MEDIASERVERCONN = 14; // /<
	...
	public static final int BRAS_USERINFO_CTRLCODE_KICKOUT = 1; // /<
																// 将指定用户从系统中踢掉
	public static final int BRAS_USERINFO_CTRLCODE_SYNCDATA = 2; // /<
																	// 将指定用户的数据同步给客户端
	public static final int BRAS_USERINFO_CTRLCODE_FUNCCTRL = 3; // /<
																	// 客户端功能控制，wParam为功能参数组合
	public static final int BRAS_USERINFO_CTRLCODE_ADDGROUP = 20; // /<
																	// 添加用户分组，wParam为分组Id，lpStrValue为分组名称
	public static final int BRAS_USERINFO_CTRLCODE_DELGROUP = 21; // /<
																	// 删除用户分组，wParam为分组Id
	public static final int BRAS_USERINFO_CTRLCODE_ADDFRIEND = 22; // /<
																	// 添加用户好友，wParam为好友Id
	public static final int BRAS_USERINFO_CTRLCODE_DELFRIEND = 23; // /<
																	// 删除用户好友，wParam为好友Id
	public static final int BRAS_USERINFO_CTRLCODE_SETGROUPRELATION = 24; // /<
																			// 设置好友与分组的关联关系，wParam为分组Id，lParam为好友Id，表示好友属于某个分组

	// 内核参数控制（API：SetSDKOption 传入参数）
	public static final int BRAS_SO_GETTRANSBUFTIMESTAMP = 1; // /<
																// 获取透明通道时间戳（传入参数为userid）
	public static final int BRAS_SO_RECORD_VIDEOBR = 2; // /<
														// 录像视频码率设置（参数为：int型，单位：bps）
	public static final int BRAS_SO_RECORD_AUDIOBR = 3; // /<
														// 录像音频码率设置（参数为：int型，单位：bps）
	public static final int BRAS_SO_RECORD_FILETYPE = 4; // /<
															// 录制文件类型设置（参数为：int型，
															// 0 MP4[默认], 1 WMV,
															// 2 FLV, 3 MP3）
	public static final int BRAS_SO_RECORD_WIDTH = 5; // /<
														// 录制视频宽度设置（参数为：int型，如：320）
	public static final int BRAS_SO_RECORD_HEIGHT = 6; // /<
														// 录制文件高度设置（参数为：int型，如：240）
	public static final int BRAS_SO_RECORD_FILENAMERULE = 7; // /<
																// 录制文件名命名规则（参数为：int型）
	public static final int BRAS_SO_RECORD_CLIPMODE = 8; // /<
															// 录制视频裁剪模式（参数为：int型）
	...
	public static final int ANYCHAT_RECORD_FLAGS_SERVER = 0x00000004; // /<
																		// 服务器端录制
	public static final int ANYCHAT_RECORD_FLAGS_MIXAUDIO = 0x00000010; // /<
																		// 录制音频时，将其它人的声音混音后录制
	public static final int ANYCHAT_RECORD_FLAGS_MIXVIDEO = 0x00000020; // /<
																		// 录制视频时，将其它人的视频迭加后录制
	public static final int ANYCHAT_RECORD_FLAGS_ABREAST = 0x00000100; // /<
																		// 录制视频时，将其它人的视频并列录制
	public static final int ANYCHAT_RECORD_FLAGS_STEREO = 0x00000200; // /<
																		// 录制音频时，将其它人的声音混合为立体声后录制
	public static final int ANYCHAT_RECORD_FLAGS_SNAPSHOT = 0x00000400; // /< 拍照
	public static final int ANYCHAT_RECORD_FLAGS_LOCALCB = 0x00000800; // /<
																		// 触发本地回调

	// 视频裁剪模式定义
	public static final int ANYCHAT_VIDEOCLIPMODE_AUTO = 0; // /<
															// 默认模式，以最大比例进行裁剪，然后再整体拉伸，画面保持比例，但被裁剪画面较大
	public static final int ANYCHAT_VIDEOCLIPMODE_OVERLAP = 1; // /<
																// 重叠模式，只取最大有效部分，对边缘进行裁剪
	public static final int ANYCHAT_VIDEOCLIPMODE_SHRINK = 2; // /<
																// 缩小模式，缩小到合适的比例，不进行裁剪
	public static final int ANYCHAT_VIDEOCLIPMODE_STRETCH = 3; // /<
																// 平铺模式，不进行裁剪，但可能导致画面不成比例
	public static final int ANYCHAT_VIDEOCLIPMODE_DYNAMIC = 4; // /<
	...
	// 业务对象参数控制
	public static native int ObjectControl(int dwObjectType, int dwObjectId, int dwCtrlCode, int dwParam1, int dwParam2, int dwParam3, int dwParam4,
			String lpStrValue);
	...
	// 业务对象事件回调函数定义
	private int OnAnyChatObjectEventNotifyCallBack(int dwObjectType, int dwObjectId, int dwEventType, int dwParam1, int dwParam2, int dwParam3, int dwParam4,
			String lpStrParam)
	{
		if (this.event != null)
	...
	static
	{
		System.loadLibrary("native/AnyChatServerSDK");
		System.loadLibrary("native/anychatserver4java");
	}
